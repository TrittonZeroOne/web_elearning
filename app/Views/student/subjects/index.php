...
<div class="pt-6">
  <?php if (empty($subjects)): ?>
    <div class="bg-white rounded-2xl p-16 text-center shadow-sm border border-slate-100">
      <i class="fa-solid fa-book-open text-5xl text-slate-300 mb-4 block"></i>
      <p class="text-slate-500 font-medium">Belum ada mata pelajaran</p>
      <p class="text-sm text-slate-400 mt-1">Hubungi admin jika ada masalah.</p>
    </div>
  <?php else: ?>
        ...
        <a href="<?= base_url('student/subjects/' . $s['id']) ?>"
          ...
          <div class="flex items-center gap-3 mb-4">
            ...
            <div class="flex-1 min-w-0">
              ...
              <p class="text-xs text-slate-500 mt-0.5"><?= esc($s['teacher_name'] ?? '') ?></p>
            </div>
          </div>
