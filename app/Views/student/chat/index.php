...
<div class="pt-6 max-w-2xl">
  <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
    ...
    <div class="px-5 py-4 border-b border-slate-100 flex items-center gap-3">
      ...
      <div>
        <h2 class="font-semibold text-slate-800">Pesan ke Guru</h2>
        <p class="text-xs text-slate-400">Chat privat dengan guru yang mengajar kamu</p>
      </div>
    </div>

    <?php if (empty($teachers)): ?>
      <div class="text-center py-16">
        <i class="fa-solid fa-chalkboard-teacher text-3xl text-slate-300 mb-3 block"></i>
        <p class="text-sm text-slate-400">Belum ada guru terdaftar di kelasmu.</p>
      </div>
    <?php else: ?>
      <ul class="divide-y divide-slate-50">
        ...
        ?>
          <li>
            <a href="<?= base_url('student/chat/' . esc($t['id'])) ?>"
              ...
              <div class="relative flex-shrink-0">
                ...
                <?php else: ?>
                  <div class="w-11 h-11 rounded-full bg-gradient-to-br from-primary-500 to-primary-700 flex items-center justify-center text-white font-bold text-sm shadow-sm">
              ...
              <div class="flex-1 min-w-0">
                <div class="flex items-baseline justify-between gap-2">
                  <p class="text-sm font-semibold text-slate-700 truncate">
                    <?= esc($t['full_name']) ?>
                    <span class="ml-1 text-xs font-normal bg-primary-100 text-primary-600 px-1.5 py-0.5 rounded-md">Guru</span>
                  </p>
                  <?php if ($last): ?>
                ...
                <?php if (!empty($t['subjects'])): ?>
                  <p class="text-xs text-slate-400 mb-0.5"><?= esc($t['subjects']) ?></p>
                <?php endif; ?>
                <p class="text-xs truncate <?= $unread ? 'text-slate-700 font-medium' : 'text-slate-400' ?>">
                  <?php if ($last): ?>
