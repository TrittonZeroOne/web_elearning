...
<?php
$roleLabel = match($other['role'] ?? '') {
    'admin'   => 'Administrator',
    'student' => 'Siswa · ' . ($other['class_name'] ?? ''),
    default   => 'Pengguna',
};
$avatarGrad = match($other['role'] ?? '') {
    'admin'   => 'from-red-400 to-red-600',
    'student' => 'from-primary-500 to-primary-700',
    default   => 'from-slate-400 to-slate-600',
};
$labelColor = match($other['role'] ?? '') {
    'admin'   => 'text-red-500',
...
<div class="h-full flex flex-col bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
  ...
  <div class="flex items-center gap-3 px-4 py-3 border-b border-slate-100 bg-white flex-shrink-0">
    <a href="<?= base_url('teacher/chat') ?>" class="p-2 rounded-xl hover:bg-slate-100 text-slate-500 transition-colors">
      <i class="fa-solid fa-arrow-left text-sm"></i>
    </a>
    <?php if (!empty($other['avatar_url'])): ?>
      <img src="<?= esc($other['avatar_url']) ?>" class="w-9 h-9 rounded-full object-cover" alt="">
    <?php else: ?>
      <div class="w-9 h-9 rounded-full bg-gradient-to-br <?= $avatarGrad ?> flex items-center justify-center text-white font-bold text-sm flex-shrink-0">
        <?= strtoupper(substr($other['full_name'], 0, 1)) ?>
      </div>
    <?php endif; ?>
    <div class="flex-1">
      <p class="text-sm font-semibold text-slate-800"><?= esc($other['full_name']) ?></p>
      <p class="text-xs font-medium <?= $labelColor ?>"><?= $roleLabel ?></p>
    </div>
  </div>
  ...
  <div id="chatBox" class="flex-1 overflow-y-auto px-4 py-4 space-y-1"
    ...
    <?php if (empty($messages)): ?>
      <div class="flex flex-col items-center justify-center h-full text-center">
        ...
        <p class="text-xs text-slate-400 mt-1">Kirim pesan pertama ke <?= esc($other['full_name']) ?></p>
      </div>
    <?php else: ?>
    ...
    <div class="flex items-end gap-2">
      ...
      <button onclick="sendMessage()" id="sendBtn"
        class="w-10 h-10 bg-primary-600 hover:bg-primary-700 active:scale-95 rounded-xl flex items-center justify-center
               text-white transition-all shadow-sm flex-shrink-0">
...
<script>
const RECEIVER_ID = '<?= esc($other['id']) ?>';
...
function appendBubble(msg, pending=false) {
  ...
  if (!document.querySelector(`[data-date-sep="${CSS.escape(label)}"]`)) {
    ...
    sep.innerHTML = `<div class="flex-1 h-px bg-slate-200"></div><span class="text-xs text-slate-400 font-medium px-3 py-1 bg-white border border-slate-200 rounded-full">${escHtml(label)}</span><div class="flex-1 h-px bg-slate-200"></div>`;
  ...
  const readIcon = isMe ? (pending ? '<i class="fa-solid fa-circle-notch fa-spin text-white/40 text-xs"></i>' : `<i class="fa-solid ${msg.is_read?'fa-check-double text-primary-300':'fa-check text-white/60'} text-xs"></i>`) : '';
  ...
  wrap.innerHTML = `<div class="max-w-[78%]"><div class="px-4 py-2.5 rounded-2xl text-sm leading-relaxed shadow-sm ${isMe?'bg-primary-600 text-white rounded-br-sm':'bg-white text-slate-800 border border-slate-100 rounded-bl-sm'}">${escHtml(msg.message).replace(/\n/g,'<br>')}</div><p class="text-xs text-slate-400 mt-1 flex items-center gap-1 ${isMe?'justify-end':''}">${time} ${readIcon}</p></div>`;
...
async function sendMessage() {
  ...
  try {
    ...
    const res  = await fetch('<?= base_url('teacher/chat/send') ?>', {method:'POST',body:fd});
...
async function poll() {
  try {
    const res  = await fetch(`<?= base_url('teacher/chat/poll/') ?>${RECEIVER_ID}/${lastMsgId}`);
    const data = await res.json();
